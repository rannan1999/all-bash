...
app.post("/api/bots/:id/reconnect", (req, res) => {
  ...
  setTimeout(() => {
    ...
    // No need to save configs here as parameters haven't changed
    res.json(result);
  }, 1000);
});
...
function autoReconnectBots() {
  ...
  idsToReconnect.forEach((id, index) => {
    ...
    setTimeout(() => {
      ...
      // 2. Create a new connection after a 1000ms delay
      setTimeout(() => {
        const result = createBot(id, host, port, username);
        if (result.success) {
      ...
      }, 1000); // 1-second delay to ensure connection is fully closed
